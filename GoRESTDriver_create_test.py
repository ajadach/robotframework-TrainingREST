...
#     response = driver.create_user_via_user_and_password('test_name', 'male', 'test_emial11311@wp.pl', 'active')
#     assert response['id']


def test_create_user_via_token_in_url():
    response = driver.create_user_via_token_in_url('test_name', 'male', 'test_emial11533311@wp.pl', 'active')
    assert response['id']


def test_create_user_token_in_headers():
    response = driver.create_user_token_in_headers('test_name', 'male', 'test_emial5131121@wp.pl', 'active')
    assert response['id']
